...
def single_iter(VPTR_Enc, VPTR_Dec, VPTR_Disc, optimizer_G, optimizer_D, sample, device, train_flag = True):
    ...
    if train_flag:
        ...
        loss_D, loss_D_fake, loss_D_real = cal_lossD(VPTR_Disc, rec_frames, future_frames, lam_gan)
        loss_D.backward()
        optimizer_D.step()
        ...
        loss_G, loss_G_gan, AE_MSE_loss, AE_GDL_loss = cal_lossG(VPTR_Disc, rec_frames, future_frames, lam_gan)
        loss_G.backward()
        optimizer_G.step()
    else:
...
def show_samples(VPTR_Enc, VPTR_Dec, sample, save_dir, renorm_transform):
    ...
    with torch.no_grad():
        ...
        rec_future_frames = VPTR_Dec(past_gt_feats)
        # rec_future_frames = VPTR_Dec(future_gt_feats)

        N = future_frames.shape[0]
        idx = min(N, 4)
        visualize_batch_clips(future_frames[0:idx, :, ...], rec_future_frames[0:idx, :, ...], save_dir, renorm_transform, desc = 'ae')
