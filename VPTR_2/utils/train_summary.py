...
def visualize_batch_clips(gt_past_frames_batch, gt_future_frames_batch, file_dir, renorm_transform = None, desc = None):
    """
    ...
    batch = torch.cat([gt_past_frames_batch, gt_future_frames_batch], dim = -1) #shape (N, clip_length, C, H, 3W)
    batch = batch.cpu()
    N = batch.shape[0]
    for n in range(N):
        clip = batch[n, ...]
