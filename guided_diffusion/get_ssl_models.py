...
def get_model(model="dino", use_head=False):
    ...
    if model == "supervised":
        ...
        embedding_model = IJEPA_base(img_size=128, patch_size=16, in_chans=3, norm_layer=nn.LayerNorm, num_frames=22, attention_type='joint_space_time', dropout=0.1, mode="train", r=0.5, embed_dim=768, device=device,
                        # encoder parameters
                        enc_depth=4,
                        enc_num_heads=6,
                        enc_mlp_ratio=4.,
                        ...
                        pos_drop_rate=0.1,
                        time_drop_rate=0.1)
        path_partial = "output/test_5/models/partial"
        if os.path.exists(path_partial):
            print('Loading model from checkpoint...')
            checkpoint = torch.load(os.path.join(path_partial, "checkpoint.pkl"), map_location='cuda')
            encoder_state_dict = checkpoint['model_state_dict']
            # encoder_state_dict['mode'] = 'test'
            embedding_model.load_state_dict(encoder_state_dict)
        embedding_model.to('cuda')
        embedding_model.eval()
        return Wrapper_IJEPA(embedding_model)
